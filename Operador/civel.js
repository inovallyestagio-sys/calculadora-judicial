class CivelCalculator {
    constructor(container) {
        ...
        this.mockProcesso = { 
            ...
            modalidadePagamento: 'Precatório',
            leiArtigo: 'EC 113/2021',
            jurosMora: '6',
            incidenciaJurosJulho: 'sim',
            tipoDivida: 'Não Tributária',
            dataInicioCalculo: '2018-06-15',
    ...
    static getHtml() {
        return `
            <div id="formFields">
                <div class="mb-8"><p class="text-gray-600">Tipo de Cálculo > Cível / Precatórios</p></div>
                    ...
                    <div class="bg-gray-50 p-6 rounded-lg border space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 items-start">
                            <div class="md:col-span-3">
                                ...
                                <div class="flex space-x-4 mt-2"><label class="flex items-center"><input type="radio" name="modalidadePagamento" value="Precatório" class="mr-2" checked>Precatório</label><label class="flex items-center"><input type="radio" name="modalidadePagamento" value="RPV" class="mr-2">RPV (Requisição de Pequeno Valor)</label></div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Seleção Lei/Artigo</label>
                                <select id="leiArtigo" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                                    ...
                                    <option value="IPCA-E (2017-2021)">IPCA-E (2017-2021)</option>
                                    <option value="TR (2009-2017)">TR (2009-2017)</option>
                                    <option value="Art. 97/ADCT">Antigo Art. 97/ADCT</option>
                                </select>
                            </div>
                            ...
                            <div><label class="block text-sm font-medium text-gray-700 mb-2">Data do Pagamento</label><input type="date" id="dataPagamento" class="w-full px-3 py-2 border border-gray-300 rounded-lg"></div>
    ...
    simularExtracao() {
        ...
        this.container.querySelector('#leiArtigo').value = this.mockProcesso.leiArtigo;
    ...
    updateParameters() {
        const lei = this.container.querySelector('#leiArtigo').value;
        let indice = 'Não aplicável';
        if (lei.includes('SELIC')) indice = 'SELIC';
        if (lei.includes('IPCA-E')) indice = 'IPCA-E';
        if (lei.includes('TR')) indice = 'TR';
    ...
    calcularValoresCivel() {
        ...
        const valorFinal = subtotal1 * 1.15;
        
        const valorHonorarios = valorFinal * (honorariosPercent / 100);
        const totalAPagar = valorFinal + valorHonorarios;

        let tableHTML = `
            <h3 class="text-xl font-semibold text-primary-blue mb-4">Tabela Detalhada (Simulação para ${modalidade})</h3>
                ...
                <table class="calculation-table w-full border-collapse table-auto">
                    <thead>
                        <tr class="bg-gray-200 text-gray-700 uppercase text-xs leading-normal">
                            ...
                            <th class="py-3 px-4 text-right">Subtotal Atualizado</th>
                        </tr>
                    </thead>
