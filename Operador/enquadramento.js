class EnquadramentoCalculator {
    constructor(container) {
        ...
        this.mockProcesso = { processNumber: '1111111-11.2025.8.11.0001', requerentes: ["Ana da Silva", "Carlos Pereira"], valorExecutado: 90345.67, dataCitacao: '2024-01-15' };
    ...
    static getHtml() {
        return `
            <div id="formFields">
                <div class="mb-8"><p class="text-gray-600 mb-2">Tipo de Cálculo > Enquadramento Administrativo</p></div>
                ...
                <div class="summary-container mt-8 grid grid-cols-1 md:grid-cols-3 gap-6"><div class="summary-box bg-white p-6 rounded-lg shadow-lg border"><p class="text-sm text-gray-500 mb-2">VALOR EXECUTADO</p><p class="text-2xl font-bold text-primary-blue" id="valorExecutadoSummary">R$ 0,00</p></div><div class="summary-box bg-white p-6 rounded-lg shadow-lg border"><p class="text-sm text-gray-500 mb-2">VALOR ATUALIZADO (TOTAL)</p><p class="text-2xl font-bold text-primary-blue" id="valorAtualizadoSummary">R$ 0,00</p></div><div class="summary-box bg-white p-6 rounded-lg shadow-lg border"><p class="text-sm text-gray-500 mb-2">DIFERENÇA</p><p class="text-2xl font-bold text-primary-blue" id="proveitoEconomicoSummary">R$ 0,00</p></div></div>
            ...
            <template id="periodo-template">
                <div class="periodo-item grid grid-cols-1 md:grid-cols-4 gap-x-4 gap-y-2 items-end p-4 border rounded-lg bg-gray-50">
                    ...
                    <div class="md:col-span-1"><button name="remove_periodo" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg w-full"><i class="fas fa-trash"></i></button></div>
                </div>
            </template>
        `;
    }
    ...
    validateParameters(config) {
        if (!config.periodoGeral.inicio || !config.periodoGeral.fim) return { isValid: false, message: "O Período Geral (Início e Fim) deve ser preenchido." };
        if (config.periodos.length === 0) return { isValid: false, message: "Adicione pelo menos um Sub-Período de Enquadramento." };
        
        for (let i = 0; i < config.periodos.length; i++) {
            const p = config.periodos[i];
            if (!p.inicio || !p.fim || !p.nivel) return { isValid: false, message: `O Sub-Período #${i + 1} está incompleto.` };
        }
    ...
    calcularValoresEnquadramento() {
        ...
        const dataLimiteSELIC = new Date('2021-12-01T00:00:00Z');
            ...
            config.periodos.forEach(periodo => {
                ...
                while (currentDate <= endDate) {
                    ...
                    config.eventos.forEach(evento => {
                        ...
                        let processarEsteMes = (eventoConfig.tipo === 'mensal' || (eventoConfig.tipo === 'anual' && eventoConfig.mes === mes));
                        ...
                        if (processarEsteMes) {
                            ...
                            const indiceNome = currentDate < dataLimiteSELIC ? 'IPCA-E' : 'SELIC';
                            const indiceInfo = this.mockIndices[indiceNome];
                            
                            let devidoEvento = (['Salário Base', '13º Salário', 'Férias'].includes(evento)) ? valorDevidoBase : valorDevidoBase * 0.1;
                            let recebidoEvento = valorRecebidoMensal * (devidoEvento / valorDevidoBase);
                            if (['13º Salário', 'Férias'].includes(evento)) recebidoEvento = 0;
                ...
                <div class="requerente-bloco" data-requerente-nome="${nome}">
                    ...
                    <div class="overflow-x-auto rounded-lg shadow-lg">
                        <table class="calculation-table w-full border-collapse table-auto">
                            <thead><tr class="bg-gray-200 text-gray-700 uppercase text-xs leading-normal"><th class="py-3 px-4 text-left">Lei Aplicada</th><th class="py-3 px-4 text-left">Descrição</th><th class="py-3 px-4 text-left">Mês Ref.</th><th class="py-3 px-4 text-left">Nível/Classe</th><th class="py-3 px-4 text-right">Devido</th><th class="py-3 px-4 text-right">Recebido</th><th class="py-3 px-4 text-right">Diferença</th><th class="py-3 px-4 text-right">Valor Atualizado</th><th class="py-3 px-4 text-right">Juros(%)</th><th class="py-3 px-4 text-right">Índice</th></tr></thead>
                            <tbody class="bg-white text-gray-600 text-sm font-light divide-y divide-gray-200">${bodyHTML}</tbody>
                            <tfoot><tr class="bg-gray-50 text-gray-700 font-bold"><td class="py-3 px-4 text-right" colspan="4">Total (${nome})</td><td class="py-3 px-4 text-right">${this.formatToCurrency(totais.devido)}</td><td class="py-3 px-4 text-right">${this.formatToCurrency(totais.recebido)}</td><td class="py-3 px-4 text-right">${this.formatToCurrency(totais.diferenca)}</td><td class="py-3 px-4 text-right">${this.formatToCurrency(totais.atualizado)}</td><td class="py-3 px-4 text-right" colspan="2"></td></tr></tfoot>
                        </table>
                    </div>
                </div>`;
        });
